    ...
    return(
        <>
            ...
            <FullScreen handle={handle} className={`w-full p-2 flex-grow overflow-hidden ${
                ...
                <div className={`w-full p-5`} style={{ maxHeight: handle.active ? '100vh' : '75vh', overflowY: 'scroll',paddingBottom: handle.active ? '8vh' : '0'}}>
                    <Alert
                        className={`mb-2 bg-red-500`}
                        message={(<h3 className={`text-xl text-white font-semibold`}>{cardInfo.totalMendep + ' Pallet Belum Kembali ke Vuteq Lebih Dari Seminggu'}</h3>)}
