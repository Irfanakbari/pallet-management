...
export default function LapTransit() {
	...
	const fetchData = async () => {
		setLoading(true)
		try {
			const response = await axiosInstance.get('/api/transit?page=1&limit=50');
	...
	const onChange = (pagination, filters) => {
		...
		const url = `/api/transit?search=${searchParam}&customer=${customerParam || ''}&vehicle=${vehicleParam || ''}&part=${partParam || ''}&page=${pagination.current}&limit=${pagination.pageSize}`;
	...
	return (
		<>
			<Head>
				<title>Laporan Transit | PT Vuteq Indonesia</title>
			</Head>
