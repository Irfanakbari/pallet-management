...
export default function LapRiwayatMaintenance() {
	...
	const fetchData = () => {
		setLoading(true)
		axiosInstance.get(`/api/repairs/histories?page=1`).then(response => {
	...
	const saveExcel = async (e) => {
		...
		const sheet = workbook.addWorksheet("Laporan Riwayat Maintenance Pallet", {
	...
	const onChange = (pagination, filters) => {
		...
		const url = `/api/repairs/histories?search=${searchParam}&customer=${customerParam || ''}&vehicle=${vehicleParam || ''}&part=${partParam || ''}&page=${pagination.current}&limit=${pagination.pageSize}`;
	...
	return (
		<>
			<Head>
				<title>Laporan Riwayat Maintenance | PT Vuteq Indonesia</title>
			</Head>
