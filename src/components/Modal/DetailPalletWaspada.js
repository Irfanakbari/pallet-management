...
export default function DetailPalletWaspada({selected}) {
	const {setModal2} = modalState();
	const [data, setData] = useState([]);
	const componentRef = useRef(null);
	const currentDate = dayjs(); // Ambil tanggal dan waktu saat ini
	const threeWeeksAgo = currentDate.subtract(3, 'week'); // Hitung tanggal 3 minggu yang lalu
	const formattedDate3Weeks = threeWeeksAgo.format('DD MMMM YYYY HH:mm [WIB]'); // Format tanggal
	...
	const handlePrint = useReactToPrint({
		...
		documentTitle: "Laporan Pallet Tidak Bergerak",
		onAfterPrint: () => setModal2(false),
					...
					<div className="h-2/3 w-2/3 flex flex-col rounded bg-white border-4 border-base">
						<div
							...
							<div className="flex items-center gap-2">
								<FaRegWindowMaximize/>
								Detail Pallet Belum Bergerak
							</div>
							<div onClick={() => setModal2(false)} className="hover:bg-red-800 p-1">
								<ImCross size={10}/>
							</div>
						</div>
									...
									<div className="w-full flex flex-col">
										<div className={`flex gap-4 items-center print-header`}>
											...
											<div className={`flex flex-col`}>
												<h2 className="font-bold text-xl">Data Pallet Tidak Bergerak</h2>
												<h3>Tanggal : {formattedDate3Weeks + ' - ' +formattedDate}</h3>
											</div>
										</div>
										<Table
											bordered
											...
											columns={[
												...
												},
												{
													...
													render: (_, record) => (dayjs(record['updated_at']).locale('id').format('DD MMMM YYYY HH:mm'))
												},
											]}
											components={{
												body: {
